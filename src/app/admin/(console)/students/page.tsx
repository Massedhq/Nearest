  ...
  return (
    <>
      ...
      <div className="card" style={{ overflowX: "auto" }}>
        <table className="tbl">
          <thead><tr><th>Name</th><th>Email</th><th>Age</th><th>School</th><th>Class</th><th>Status</th><th>Reverify by</th><th>Joined</th></tr></thead>
          <tbody>
            {rows.length === 0 && <tr><td className="empty" colSpan={8}>No students yet.</td></tr>}
            {rows.map(({ s, u, school }) => (
              <tr key={s.userId}>
                <td>{u.firstName} {u.lastName}</td><td>{u.email}</td><td>{age(u.dateOfBirth)}</td><td>{school ?? "—"}</td><td>{s.graduationYear ?? "—"}</td>
